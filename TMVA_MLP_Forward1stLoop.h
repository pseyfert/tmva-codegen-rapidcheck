...
class ReadMLP_Forward1stLoop : public IClassifierReader {
 ...
 private:
  ...
  inline float GetMvaValue__( const std::vector<float>& inputValues ) const
  {
    ...
    //reply (Paul Seyfert): can be autovectorised with a few changes
    //                      cf  https://github.com/root-project/root/pull/121 
    //                      and https://github.com/pseyfert/tmva-mlp
     ...
     for (int o=0; o<10; o++) {
        for (int i=0; i<8; i++) {
           buffer[i] = fWeightMatrix0to1[o][i] * fWeights0[i];
        }
        for (int i=0; i<8; i++) {
     ...
     // layer 3 to 4
     for (int i=0; i<8; i++) {
       buffer[i] = fWeightMatrix3to4[0][i] * fWeights3[i];
     }
     for (int i=0; i<8; i++) {
       fWeights4[0] += buffer[i];
     }
  ...
  //reply (Paul Seyfert): this transformation is indeed not needed (provided the cut value gets adapted)
